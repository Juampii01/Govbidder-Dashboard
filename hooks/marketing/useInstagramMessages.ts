...
export function useInstagramMessages() {
  ...
  const loadConversations = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/marketing/instagram/messages')
      const json = await res.json()
      if (!res.ok) { setConvsState({ status: 'error', message: json.error ?? 'Error' }); return }
  ...
  const loadThread = useCallback(async (conversationId: string) => {
    ...
    try {
      const res = await fetch(`/api/marketing/instagram/messages?conversationId=${encodeURIComponent(conversationId)}`)
      const json = await res.json()
      if (!res.ok) { setThreadState({ status: 'error', message: json.error ?? 'Error' }); return }
  ...
  const sendMessage = useCallback(async (conversationId: string, text: string): Promise<
    { ok: true; message: IGMessage } | { ok: false; error: string }
  > => {
    setSending(true)
    try {
      const res = await fetch('/api/marketing/instagram/messages/send', {
        method: 'POST',
  ...
  const sync = useCallback(async () => {
    setSyncing(true)
    try {
      await fetch('/api/marketing/instagram/messages/sync', { method: 'POST' })
