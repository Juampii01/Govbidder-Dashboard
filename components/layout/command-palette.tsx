...
export function CommandPalette({ open, onClose, onSignOut }: CommandPaletteProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-start justify-center px-4 pt-[12vh]" onClick={onClose}>
      ...
      {/* Modal */}
      <div
        className="relative w-full max-w-[640px] overflow-hidden rounded-2xl border border-border bg-popover/95 backdrop-blur-xl shadow-[0_30px_80px_rgba(15,23,42,0.25)] page-enter"
        ...
        <div className="max-h-[60vh] overflow-y-auto py-2">
          {finalResults.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              <Search className="h-6 w-6 text-muted-foreground/60 mb-3" />
              <p className="text-sm text-muted-foreground">Sin resultados para <span className="text-foreground">"{query}"</span></p>
              <p className="text-[11px] text-muted-foreground/60 mt-1">Probá con otro término o abrí el listado.</p>
            </div>
          ) : (
            grouped.map((g, gi) => (
                    ...
                    return (
                      ...
                      >
                        ...
                        <ArrowRight className={`h-3.5 w-3.5 shrink-0 transition-all ${
                          isActive ? "text-[#ff6b6a] translate-x-0.5" : "text-muted-foreground/60"
                        }`} />
                      </button>
                    )
                  })}
