...
function StatTile({
  ...
  const palette = {
    ...
    neutral: { ring: "ring-slate-200",   icon: "bg-muted   text-muted-foreground",    glow: "rgba(15,23,42,0.05)" },
  }[accent]

  return (
    <div
      className={`relative overflow-hidden rounded-2xl ring-1 ${palette.ring} bg-card/70 backdrop-blur-sm p-4`}
    >
      <div
...
export function InicioView() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* HERO ─────────────────────────────────────────────────────────────── */}
      <div
        className="relative overflow-hidden rounded-3xl border border-border/80 dark:border-[#1e3a8a]/30 bg-card dark:bg-[#0d1745]"
      ...
      >
        ...
        <div className="relative grid lg:grid-cols-[1fr_auto] gap-8 p-7 sm:p-9 items-center">
          ...
          {/* LEFT — title + message */}
          <div className="min-w-0">
            <div className="inline-flex items-center gap-2 mb-4 rounded-full border border-border/80 bg-card/70 backdrop-blur-sm pl-1.5 pr-3 py-1">
        ...
        <div className="relative border-t border-border bg-muted/40 grid grid-cols-2 sm:grid-cols-4">
          {[
            ...
            return (
              <div
                key={s.label}
                className={`relative px-5 py-4 ${i < 3 ? "sm:border-r border-border/70" : ""} ${i < 2 ? "border-b sm:border-b-0 border-border/70" : ""}`}
              >
            ...
            {departments.map(d => {
              ...
              return (
                <Link
                  ...
                  className="group relative overflow-hidden rounded-2xl border border-border bg-card p-4 transition-all hover:border-border hover:shadow-[0_0_24px_rgba(15,23,42,0.04)]"
                >
                ...
                <div className="divide-y divide-slate-100">
                  {overdueTasks.slice(0, 5).map(t => (
                    ...
                    >
                      ...
                      <ArrowRight className="h-3.5 w-3.5 text-slate-300 group-hover:text-muted-foreground group-hover:translate-x-0.5 transition-all" />
                    </Link>
                  ))}
            ...
            return (
              ...
              >
                ...
                <div className="relative flex items-start justify-between gap-3 mb-3">
                  <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-muted ring-1 ring-slate-200 group-hover:bg-[#1e3a8a]/10 group-hover:ring-[#1e3a8a]/25 transition-colors">
