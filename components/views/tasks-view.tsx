  ...
  return (
    ...
    >
      <button
        {...attributes}
        {...listeners}
        className="shrink-0 cursor-grab active:cursor-grabbing text-slate-300 hover:text-muted-foreground transition-colors"
              ...
              <div className="space-y-2 pt-2 border-t border-border">
                ...
                <form onSubmit={submitSub} className="flex gap-2">
                  ...
                  <button
                    ...
                    className="flex h-8 w-8 items-center justify-center rounded-lg bg-muted text-muted-foreground hover:bg-slate-200 hover:text-foreground disabled:opacity-30 transition-all"
                  >
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="rounded-full bg-card/70 px-2 py-0.5 text-[10px] font-bold tabular-nums text-muted-foreground">
          {count}
        </span>
      </div>
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <Plus className="h-3.5 w-3.5" />
              Nueva tarea
              <kbd className="hidden sm:inline ml-1 rounded bg-card/20 px-1 text-[10px]">Q</kbd>
            </button>
          </div>
        </div>
            ...
            {savedViews.map(v => (
              <div key={v.id} className="group inline-flex items-stretch rounded-full border border-border bg-card overflow-hidden hover:border-[#1e3a8a]/30 transition-colors">
                ...
                <button
                  ...
                  className="px-1.5 h-7 text-slate-300 hover:text-red-600 hover:bg-red-50 transition-colors border-l border-border opacity-0 group-hover:opacity-100"
                  title="Borrar vista"
                >
                  <X className="h-3 w-3" />
                </button>
                      ...
                      >
                        <td className="px-3 py-3 whitespace-nowrap" onClick={e => e.stopPropagation()}>
                          <button onClick={() => toggleSelect(t.id)} className="flex items-center justify-center">
                            {isSelected
                              ? <CheckSquare className="h-3.5 w-3.5 text-[#1e3a8a]" />
                              : <Square className="h-3.5 w-3.5 text-slate-300 group-hover:text-muted-foreground" />}
                          </button>
                        </td>
