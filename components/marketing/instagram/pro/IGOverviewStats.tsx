...
function Card({
  ...
  return (
    ...
    >
      {highlight && (
        <div className={`absolute inset-0 ${IG_GRADIENT_CSS} opacity-10 pointer-events-none`} />
      )}
      <div
        className={`w-8 h-8 rounded-xl flex items-center justify-center mb-3 ${highlight ? IG_GRADIENT_CSS : 'bg-[var(--muted)]'}`}
      >
        <I size={15} className={highlight ? 'text-white' : 'text-[var(--muted-foreground)]'} strokeWidth={1.5} />
      </div>
...
export function IGOverviewStats({ summary }: { summary: InstagramAccountSummary }) {
  ...
  return (
    <div className="flex gap-3 overflow-x-auto pb-1">
      <Card icon={Users}     label="Seguidores"    value={fmt(snap?.followers)}   highlight />
      <Card icon={Eye}       label="Views totales"  value={fmt(snap?.totalViews)} />
      <Card icon={TrendingUp} label="Engagement"   value={er != null ? er.toFixed(2) + '%' : '—'} sub={erLabel} />
      <Card icon={Compass}   label="Alcance"        value={fmt(snap?.reach)} />
      <Card icon={FileText}  label="Posts"          value={fmt(snap?.posts)} />
    </div>
  )
}
