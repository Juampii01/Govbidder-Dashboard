        ...
        {top3.map((r, i) => (
          <div key={r.id} className="flex-1 min-w-0">
            <div className="relative aspect-[3/4] rounded-xl overflow-hidden bg-[var(--muted)] mb-2">
              ...
              <div className="absolute top-1.5 left-1.5 text-base drop-shadow">{MEDALS[i]}</div>
            ...
            <div className="flex items-center justify-between px-0.5">
              <div className="flex items-center gap-0.5">
                <Heart size={10} className="text-[var(--muted-foreground)]" />
                <span className="text-[11px] text-[var(--muted-foreground)]">{fmt(r.likesCount)}</span>
              </div>
              <div className="flex items-center gap-0.5">
                <MessageCircle size={10} className="text-[var(--muted-foreground)]" />
                <span className="text-[11px] text-[var(--muted-foreground)]">{fmt(r.commentsCount)}</span>
              </div>
            </div>
          </div>
        ))}
