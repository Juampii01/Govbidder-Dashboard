      ...
      <div className="flex items-center justify-between py-3 px-1 mb-1">
        <div className="flex items-center gap-1 flex-wrap">
          {(['recent', 'views', 'likes', 'comments'] as Sort[]).map((s) => (
            ...
            >
              {s === 'recent' ? 'Reciente' : s === 'views' ? '👁 Views' : s === 'likes' ? '❤️ Likes' : '💬 Coms.'}
            </button>
          ))}
        </div>
        <div className="flex gap-1">
          <button
            ...
            className={`p-1.5 rounded transition-colors ${view === 'grid' ? 'text-[var(--foreground)]' : 'text-[var(--muted-foreground)]'}`}
          >
            <Grid3X3 size={16} />
          </button>
          <button
            onClick={() => setView('list')}
            className={`p-1.5 rounded transition-colors ${view === 'list' ? 'text-[var(--foreground)]' : 'text-[var(--muted-foreground)]'}`}
          >
            <List size={16} />
          </button>
        </div>
            ...
            >
              ...
              <div className="flex gap-3 flex-shrink-0">
                {(
                  ...
                  ] as { I: React.ElementType; v: number }[]
                ).map(({ I, v }, i) => (
